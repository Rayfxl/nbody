...
__global__ void bodyForce(Body* p, float dt, int n) {
    ...
    int k = blockIdx.x + blockIdx.x / numBlocks;
...
__global__ void updatePositions(Body* p, float dt, int n) {
...
int main(const int argc, const char** argv)
{
    ...
    int numBlocks = nBodies / BLOCK_SIZE;
    ...
    for (int iter = 0; iter < nIters; iter++)
    {
         ...
         */
         ...
         // 调用CUDA内核
        bodyForce << <numBlocks * BLOCK_STEP, BLOCK_SIZE >> > (p, dt, nBodies);
        cudaDeviceSynchronize(); // 确保内核执行完成
        updatePositions << <numBlocks, BLOCK_SIZE >> > (p, dt, nBodies);
        cudaDeviceSynchronize(); // 确保内核执行完成
